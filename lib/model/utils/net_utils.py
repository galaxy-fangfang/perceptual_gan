...
def vis_detections(im, class_name, dets, thresh=0.8):
    ...
    for i in range(np.minimum(10, dets.shape[0])):
        ...
        score = dets[i, -1]
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], class_color[class_name], 2)
            cv2.putText(im, '%s: %.3f' % (class_name, score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN,
                        1.0, (0,0,255), thickness=1)
    cv2.rectangle(im, (421, 499), (475,531), class_color['nest'], 2)
    cv2.putText(im, '%s: %.3f' % ('nest', 0.632), (421, 499 + 15), cv2.FONT_HERSHEY_PLAIN,
                1.0, (0,0,255), thickness=1)

    return im


def adjust_learning_rate(optimizer_G,decay=0.1):
